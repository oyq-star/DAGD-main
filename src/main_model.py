...
class DGAD_base(nn.Module):
    def __init__(self, target_dim, config, device,ratio = 0.7):
        ...
        print('DGAD_Physio_sum: ', sum(p.numel() for p in self.diffmodel.parameters()))
...
class DGAD_PM25(DGAD_base):
    def __init__(self, config, device, target_dim=36):
        super(DGAD_PM25, self).__init__(target_dim, config, device)
...
class DGAD_Physio(DGAD_base):
    def __init__(self, config, device, target_dim=35,ratio = 0.7):
        super(DGAD_Physio, self).__init__(target_dim, config, device,ratio)
